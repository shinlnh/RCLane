"""
Datasets that turn lane annotations into RCLane training tensors.

`LaneEncodeDataset` holds everything shared across annotation formats: image
resize + normalization, running `encode` to build the 5 GT maps, and caching that
GT to disk in SPARSE form (foreground pixels only) so we don't recompute the slow
shapely encode every epoch, nor store hundreds of GB of dense maps.

Subclasses only implement `_load(idx)`, returning the raw image and lane polylines
in the original image coordinate space.

  - `CarlaLaneDataset`  : the CARLA LaneATT JSONL format (BanVienCorp dataset).
  - CULane is in `dataset_culane.py`.
"""
...
class CarlaLaneDataset(LaneEncodeDataset):
    """CARLA LaneATT JSONL format (BanVienCorp/dataset_laneatt_fullmap)."""
...
RCLaneDataset = CarlaLaneDataset
...
if __name__ == "__main__":
    ...
    root = sys.argv[1] if len(sys.argv) > 1 else "../RCLane/data/dataset"
    ds = CarlaLaneDataset(
        label_json=os.path.join(root, "label_val.json"),
        data_root=root,
        cache_dir="./gt_cache_val",
        max_samples=3,
    )
    print(f"dataset size (capped): {len(ds)}")
    x, tgt = ds[0]
    print("image:", tuple(x.shape), x.dtype, f"[{x.min():.2f}, {x.max():.2f}]")
    print("seg fg pixels:", int((tgt["seg_map"] > 0).sum()))
    for k, v in tgt.items():
        print(f"  {k:11s} {tuple(v.shape)} {v.dtype}")
    xb, tb = collate([ds[0], ds[1]])
    print("batched image:", tuple(xb.shape), "| seg:", tuple(tb["seg_map"].shape))
    print("OK -- dataset yields tensors, collate batches them.")
